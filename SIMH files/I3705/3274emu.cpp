...
#define I3274EMUTHREAD 0
#define I3705SCANNERTHREAD 1

int socketpairFds[2];

void *Run3274Emu (void *arg) {
  int sock;
  ...
  while (1) {
    ...
    if (newsock == -1) {
      ...
      handle(newsock, socketpairFds[I3274EMUTHREAD]);
    }
  }
...
int start3274Emu () {
  ...
  fcntl(socketpairFds[1], F_SETFL, flags | O_NONBLOCK);    
  rc = pthread_create(&thread, NULL, Run3274Emu, NULL);
